...
internals.Client.prototype.request = function (method, url, options, callback, _trace, _responses) {
    ...
    const hasTransferEncoding = Object.keys(uri.headers).some((key) => {
    ...
    if((uri.method === 'GET' || uri.method === 'HEAD') && (hasContentLength || hasTransferEncoding || options.payload)){
        // remove payload and content-length to stop HPE_INVALID_EOF_STATE error
        Object.keys(uri.headers).forEach( (key) => {
            if(key.toLowerCase() === 'content-length' || 'content-type' || 'transfer-encoding'){
                delete uri.headers[key];
            };
        })
    ...
    const onResponse = (res) => {
        ...
        if (redirects === 0) {
            return finishOnce(Boom.badGateway('Maximum redirections reached', _trace, _responses));
        }

        if (!location) {
            return finishOnce(Boom.badGateway('Received redirection without location', _trace, _responses));
        }
        ...
        const redirectReq = this.request(redirectMethod, location, redirectOptions, finishOnce, _trace, _responses);
